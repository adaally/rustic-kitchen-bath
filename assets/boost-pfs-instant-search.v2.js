...
(function() {
  ...
  /* Fix input doesn't focus in Brooklyn theme */
  InstantSearchStyle3.prototype.afterCloseInstantSearchStyle3 = function(isClose) {
    jQ('.mfp-close').trigger('click');
  }

  //preserve original search term after loading from our app
  var inputValue;
  SearchInput.prototype.beforeRender = function() {
    inputValue = this.$element.val()
  }
  SearchInput.prototype.afterRender = function() {
    var currentValue = this.$element.val(); 
    if (!currentValue && inputValue) this.$element.val(inputValue);
  }

  jQ(document).on('click','.icon_search',function(){
    if(!jQ('body').hasClass('boost-pfs-enable-instant-search')){
      boostPFS.initSearchBox();
    }
  });
